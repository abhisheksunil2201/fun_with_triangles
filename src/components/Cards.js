...
function Cards() {
  ...
  function optionClicked(e, option) {
    document
      .querySelector(".selectedOption")
      ?.classList.toggle("selectedOption");
    e.target?.classList.toggle("selectedOption");
      ...
      {index >= 0 && (
        <form className="cards" onSubmit={saveResponse}>
          ...
          <div className="options">
            <div className="option 0" onClick={(e) => optionClicked(e, 0)}>
              Right-angled
            </div>
            <div className="option 1" onClick={(e) => optionClicked(e, 1)}>
              Acute-angled
            </div>
            <div className="option 2" onClick={(e) => optionClicked(e, 2)}>
              Obtuse-angled
            </div>
            <div className="option 3" onClick={(e) => optionClicked(e, 3)}>
              Not a triangle
            </div>
          </div>
          <div className={`quiz-btn ${nextVisible}`} onClick={changeIndex}>
            Next
          </div>
          <button className={`end-game ${submitVisible}`} type="submit">
            Submit
          </button>
        </form>
      )}
